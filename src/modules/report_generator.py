...
def generar_pdf_clinico(paciente, proc, resultados_inferencia):
    ...
    pdf.cell(95, 6, f'* Diagnostico de Referencia: {paciente["etiqueta_referencia"]}', ln=True)
    pdf.cell(95, 6, f'* Frecuencia Muestreo Nativa: {paciente["frecuencia_muestreo"]} Hz', ln=False)
    pdf.cell(95, 6, f'* Duracion Total: {duracion_seg:.2f} segundos', ln=True)
    pdf.cell(95, 6, f'* Canales Totales en Matriz: {paciente["num_derivaciones"]} derivaciones', ln=False)
    pdf.cell(95, 6, f'* Fecha/Hora de Auditoria: {datetime.datetime.now().strftime("%d/%m/%Y %H:%M")}', ln=True)
    pdf.ln(5)
    
    # 2. CONFIGURACIÓN DEL PREPROCESAMIENTO
    pdf.set_font('Helvetica', 'B', 11)
    pdf.cell(0, 8, '2. ESPECIFICACIONES DEL PIPELINE DE PREPROCESAMIENTO', ln=True)
    pdf.line(10, pdf.get_y(), 115, pdf.get_y())
    pdf.ln(3)
    
    pdf.set_font('Helvetica', '', 10)
    pdf.cell(95, 6, f'* Derivacion de Diseno Analizada: {proc.get("derivacion_procesada", "II")}', ln=False)
    ...
    pdf.cell(95, 6, f'* Normalizacion de Amplitud: Z-score (Media 0, Varianza 1)', ln=True)
    pdf.cell(95, 6, f'* Ventana de Segmentacion Rigida: {proc["cantidad_segmentos"]} bloques de 10 segundos', ln=True)
    pdf.ln(5)
    
    # 3. TABLA DE INFERENCIA POR SEGMENTOS
    pdf.set_font('Helvetica', 'B', 11)
    pdf.cell(0, 8, '3. ANALITICA DE PREDICCIONES PROBABILISTICAS DE LA IA (SOFTMAX)', ln=True)
    pdf.line(10, pdf.get_y(), 135, pdf.get_y())
    pdf.ln(4)
    ...
    pdf.cell(45, 7, 'Prediccion Ganadora', border=1, align='C', fill=True)
    ...
    pdf.cell(0, 4, 'Validacion y Control de Calidad del Diagnostico por Computadora', ln=True, align='C')
